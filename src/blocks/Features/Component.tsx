  ...
  return (
    <div className="container my-16">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {items.map((item, i) => (
          isMinimal ? (
            <div key={i} className="border-l-2 border-primary/40 pl-5 pr-3 py-1">
              <h3 className="font-semibold mb-2">{item.title}</h3>
              {item.description && <RichText data={item.description} enableGutter={false} />}
            </div>
          ) : (
            <SurfaceCard key={i} className="rounded-lg border-0 p-6 shadow-sm shadow-black/10 dark:shadow-black/40">
              <h3 className="font-semibold mb-2">{item.title}</h3>
              {item.description && <RichText data={item.description} enableGutter={false} />}
            </SurfaceCard>
          )
